...
1. **调参数**：填写行程、日期、开售时间和乘车人，选择可接受的席别与尝试顺序；需要时填写优先车次、座位或铺位偏好。点击“检查配置”确认无误。
2. **开始任务**：点击“开始任务”，程序开始准备登录，随后在界面中显示二维码。
3. **扫二维码**：打开 **12306 App** 扫描程序中的二维码，并在手机上确认登录。
4. **等待**：保持程序运行，等待开售、查询余票和提交结果。提交成功后按提示前往 12306 完成支付。

程序不弹出 Windows 系统通知，也不播放提示音；请留意窗口中的状态、日志和结果提示。
...
- 🚆 **按你的顺序抢**：自动识别车次类型，可切换车次优先或席别优先，已选席别集中拖动排序。
...
```text
12306FairTicket-1.1.1-Windows-x64.zip
```
...
按上方的 **调参数 → 开始任务 → 扫二维码 → 等待** 操作。二维码会在点击“开始任务”后生成，无需在填写参数前登录。
...
在“卧铺”菜单中选择高级软卧、软卧、硬卧、一等卧或二等卧，再设置需要几个下铺、中铺和上铺。例如两位乘车人可以选择：
...
一等卧、二等卧是独立席别，按 12306 返回的实际席别代码识别；若同时接受普通列车的软卧、硬卧，请分别勾选。是否支持选铺和中铺以服务端返回为准。
