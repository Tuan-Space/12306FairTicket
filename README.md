...
GUI 支持二维码直接显示及有效期倒计时、服务器校时后的开售倒计时、日历与时/分/秒输入、字段级校验、可拖动的席别优先级、整组座位关系和上/中/下铺软偏好、协作停止、任务时间线及异步脱敏日志。“保存为…”与“导入…”使用 version 2 JSON 保存全部可编辑参数；Cookie、Token、二维码、身份信息和运行路径永不写入配置。GUI 每次启动均重新扫码，Cookie 与二维码不会写入磁盘。
